...
metatests.case(
  ...
  { common },
  {
    ...
    'common.mergeObjects': [
      ...
      [
        (key) => key,
        Object.assign(Object.create(null), { 1: 'a' }),
        { 1: '1' },
      ],
    ],
  },
...
metatests.test('clone handling only object own properties', (test) => {
  ...
  const res = common.clone(obj);
  test.strictSame(res, obj);
  test.end();
});

metatests.testSync('mergeObjects correctly handles ownProperties', (test) => {
  const objects = [{ a: 1 }, { a: 2 }];
  Object.setPrototypeOf(objects[1], { b: 3 });
  const actual = common.mergeObjects((_, a, b) => a + b, ...objects);
  test.strictSame(actual, { a: 3 });
});

metatests.testSync(
  'mergeObjects passes correct arguments to merger',
  (test) => {
