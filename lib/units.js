...
const durationToString = (
  ...
  n = Math.floor(n / 1000);
  const parts = [];
  let unit, remainder, mult;
  for (unit in UNITS_MAX_DURATION) {
    mult = UNITS_MAX_DURATION[unit];
    remainder = n % mult;
...
const bytesToSize = (
  // Convert int to string, representing file size in Kb, Mb, Gb and Tb
  bytes // number, size
  // Returns: string
) => {
  if (bytes === 0) return '0';
...
const sizeToBytes = (
  // Convert string with file size to int
  size // string, size
  // Returns: number
) => {
