...
class Iterator {
  ...
  // keeper.toArray(); // [3, 4]
  keepLast() {
    return new KeepLastIterator(this);
  }
...
class KeepLastIterator extends Iterator {
  constructor(base) {
    super(base);
    this.repeat = false;
  ...
  // Returns: <any> last stored value or `undefined`
  last() {
    if (this.lastElement === undefined || this.lastElement.done) {
      return undefined;
    }
    return this.lastElement.value;
  }

  // Makes this iterator return last stored value upon next `next` call
  // Returns: <this>
  repeatLast() {
    this.repeat = true;
    return this;
  ...
  next() {
    if (this.repeat && this.lastElement !== undefined) {
      this.repeat = false;
      return this.lastElement;
    }
    this.lastElement = this.base.next();
    return this.lastElement;
  }
}
