...
function getPlatformDelimiter() {
  ...
  return systems[opSys] ? systems[opSys] : systems.unix;
}
...
function checkValue(value, type, options) {
  ...
  const _quotedString = quotedString && type === 'string';
  let _quotedMatch =
    quotedMatch &&
    type === 'string' &&
  ...
  _quotedMatch = quotedMatch && quotedMatch.length > 0;
  const shouldQuote =
    containsQuote === true ||
    containsDelimiter ||
...
class Options {
  ...
  valueToString(incomingValue, context) {
    if (incomingValue === undefined) incomingValue = '';
    context.column = this.header
      ? this.columns[context.index].key
      : context.index;
    const type = incomingValue instanceof Date ? 'date' : typeof incomingValue;
    let outputValue = this._cast[type](incomingValue, context);
      ...
      if (shouldQuote === true && containsEscape === true) {
        regexp =
          escape === '\\'
            ...
            : new RegExp(escape, 'g');
        outputValue = outputValue.replace(regexp, escape + escape);
      }
      if (containsQuote === true) {
        regexp = new RegExp(quote, 'g');
        outputValue = outputValue.replace(regexp, escape + quote);
      }
      if (shouldQuote === true) {
        outputValue = quote + outputValue + quote;
      }
    } else if (
  ...
  columnsToString() {
    ...
    for (const index in this.columns) {
      ...
      output += this.valueToString(this.columns[index].header, context);
    }
    return output;
  }
}
...
function createRecord(data, record, options) {
  ...
  if (Array.isArray(data)) {
    ...
    for (const index in data) {
      context.index = index;
      output += options.valueToString(data[index], context);
      if (index !== lastValue) output += options.delimiter;
    }
  } else if (typeof data === 'object') {
    const lastValue = options.columns.length - 1;
    for (const index in options.columns) {
      const { key } = options.columns[index];
      context.index = index;
      output += options.valueToString(data[key], context);
      if (index !== lastValue) output += options.delimiter;
    }
  } else {
...
function stringify(...args) {
  if (args.length < 2) throw new Error('2 arguments is required');
  else if (args.length > 3)
    throw new Error('Not more than 3 arguments is required');
  ...
  try {
    ...
    const options =
      args.length === 3 ? Options.create(args[1]) : Options.create();
