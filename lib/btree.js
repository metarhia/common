...
const binarySearch = (node, key) => {
  ...
  while (start <= end) {
    ...
    if (key > itemValue) start = i + 1;
    // the biggest element in the node has key === undefined, therefore,
    // in order to get true for it here using reverse comparison.
    else if (!(key >= itemValue)) end = i - 1;
    else return [true, i];
  }
  return [false, start];
};
...
function* inorderTraversal(start, finish, currNode) {
  const binarySearchResult = binarySearch(currNode, start);
  let startIndex = binarySearchResult[1];
  if (binarySearchResult[0]) startIndex++;
  const finishIndex = finish === undefined ?
    currNode.length - 1 :
